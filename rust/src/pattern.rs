...
unsafe impl Pattern for Alphabetic {
    fn prefix_length_of(&self, haystack: &str) -> Option<usize> {
        match haystack.chars().next() {
            Some(c) if c.is_alphabetic() => Some(c.len_utf8()),
            _ => None,
        }
    }
}
...
unsafe impl Pattern for Alphanumeric {
    fn prefix_length_of(&self, haystack: &str) -> Option<usize> {
        match haystack.chars().next() {
            Some(c) if c.is_alphanumeric() => Some(c.len_utf8()),
            _ => None,
        }
    }
}
...
#[derive(Clone, Copy)]
pub struct Sequence<'a>(&'a [&'a dyn Pattern]);
